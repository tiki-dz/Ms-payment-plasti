...
const { MESSAGE_BROKER_URL, PAYMENT_BINDING_KEY } = require('../config/config.js')
...
module.exports.CreatChannel = () => {
  amqp.connect(MESSAGE_BROKER_URL, function (error0, connection) {
    ...
    connection.createChannel(function (error1, channel) {
      ...
      channel.assertQueue(PAYMENT_BINDING_KEY, {
        durable: false
      })
      channel.consume(PAYMENT_BINDING_KEY, async function (msg) {
...
// TODO update this section HARIRI ALI
async function eventSwitcher (event, payload) {
  switch (event) {
    case 'event name':
      await eventHendler.functionName(payload)
      break
